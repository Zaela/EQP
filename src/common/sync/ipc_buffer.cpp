...
bool IpcRemote::pop(IpcPacket& in)
{
    return m_ipcBuffer->remote().pop(in);
}

void IpcRemote::push(ServerOp opcode, int sourceId, uint32_t len, const void* data)
{
    if (m_ipcBuffer->master().push(opcode, sourceId, len, (const byte*)data))
    {
...
void IpcRemote::pushThreadSafe(ServerOp opcode, int sourceId, uint32_t len, const void* data)
{
    std::lock_guard<AtomicMutex> lock(m_outQueueMutex);
    m_outQueue.emplace_back(opcode, sourceId, len, (const byte*)data);
}
